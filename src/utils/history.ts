// ─────────────────────────────────────────────────────────────────────────
// DEMO / TEST DATA GENERATOR - not used in normal operation.
//
// This deterministically fabricates a few months of sample tasks so the
// Progress calendar has something to show when demoing or testing the app.
// It is OFF by default; real users start with an empty calendar.
//
// To turn it on: open the app with `?demo=1` (works on the live URL too), or
// run `localStorage.setItem("barely:demo","1")` in the console, then reload.
// Turn it off + wipe the sample data with `?demo=0`.
//
// Wiring lives in `src/stores/taskStore.ts` (see `demoEnabled` / `seedHistory`).
// ─────────────────────────────────────────────────────────────────────────
